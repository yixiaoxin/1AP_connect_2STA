...
int mbedtls_client_close(MbedTLSSession *session)
{
    ...
    if(session->buffer){
        mbedtls_free(session->buffer);
		session->buffer = TLS_NULL;
    }
	
    if(session->host){
        mbedtls_free(session->host);
		session->host = TLS_NULL;
    }
    
    if(session->port){
        mbedtls_free(session->port);
...
int mbedtls_client_context(MbedTLSSession *session)
{
    ...
    //mbedtls_ssl_conf_read_timeout(&session->ssl, 15000);
...
int mbedtls_client_connect(MbedTLSSession *session)
{
    ...
    mbedtls_ssl_conf_read_timeout(&(session->ssl), 5*1000);
