...
TimerHandle_t g_test_timer_handle;
uint32_t g_test_timer_period;
...
void test_timer_handler(void const *param)
{
    const ticker_data_t *us_tick_d = get_us_ticker_data();
    const ticker_data_t *lp_tick_d = get_lp_ticker_data();
    uint64_t us_time_us = ticker_read_us(us_tick_d);
    uint64_t lp_time_us = ticker_read_us(lp_tick_d);
    struct tm tmr = {0,};
    uint32_t sec, usec;
    ...
    dbg("rtos tick: %d, us: %08x,%08x, lp: %08x,%08x\n", rtos_tick, (uint32_t)(us_time_us >> 32), (uint32_t)us_time_us, (uint32_t)(lp_time_us >> 32), (uint32_t)lp_time_us);
}

void test_timer_start(int func)
{
    if (func == 1) { // update rtc epoch time
        struct tm tmr = {0,};
        time_t gmt;
        ...
        dbg("Update time = %d-%02d-%02d %02d:%02d:%02d %d\n",tmr.tm_year+YEAR0,tmr.tm_mon+1,tmr.tm_mday,
                tmr.tm_hour,tmr.tm_min,tmr.tm_sec,(uint32_t)gmt);
    } else if (func == 0) { // start timer
        g_test_timer_handle = rtos_timer_create("tick_timer", 1000, pdTRUE, NULL,
                          (TimerCallbackFunction_t)test_timer_handler);
        if (g_test_timer_handle) {
            rtos_timer_start(g_test_timer_handle, 0, false);
        } else {
            dbg("test timer create fail\n");
        }
    } else {
        dbg("invalid func: %d\n", func);
    }
...
int cmd_timer_start(int argc, char * const argv[])
{
    unsigned int func = 0;
    if (argc > 1) {
        func = console_cmd_strtoul(argv[1], NULL, 10);
    }
    test_timer_start(func);
    return 0;
}
...
void ticker_test(void)
{
    ...
    console_cmd_add("start",    "- start timer",        1, 2, cmd_timer_start);
