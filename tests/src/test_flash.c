...
static int cmd_flash_reg_cfg(int argc, char * const argv[])
{
    ...
    if (func == 0) {
        ...
        unsigned int rd_val = flash_status_register_read(1);
        UART_PRINT("status reg rd: %x\n", rd_val);
    }
    else if (func == 3) {
        if (argc > 3) {
            unsigned int wr_val = console_cmd_strtoul(argv[2], NULL, 16);
            unsigned int wr_msk = console_cmd_strtoul(argv[3], NULL, 16);
            unsigned int rd_val;
            flash_status_register_write(wr_val, wr_msk);
            rd_val = flash_status_register_read(1);
            UART_PRINT("status reg wr: %x(m:%x), rd: %x\n", wr_val, wr_msk, rd_val);
        } else {
            UART_PRINT("err argc: %d\n", argc);
        }
    }
    return 0;
}

static int cmd_flash_unique_id_rd(int argc, char * const argv[])
{
    char uid_array[FLASH_UNIQUE_ID_LEN] = {0,};
    flash_unique_id_read(0, FLASH_UNIQUE_ID_LEN, &uid_array[0]);
    UART_PRINT("flash unique id:\n");
    dump_mem((uint32_t)uid_array, FLASH_UNIQUE_ID_LEN, 1, false);
    return 0;
}
#endif
...
void flash_test(void)
{
    ...
    #if (PLF_AIC8800M40)
    console_cmd_add("cfgsr",        "cfg status reg",         2, 4, cmd_flash_reg_cfg);
    console_cmd_add("ruid",         "read unique id",         1, 1, cmd_flash_unique_id_rd);
    #endif
