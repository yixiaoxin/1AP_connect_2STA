...
static uint32_t creating_task_depth = 0;
static enum rtos_task_id creating_task_id = UNDEF_TASK;
static struct task_stack_info task_stack_table[MAX_TASK];
#endif
                     ...
                     rtos_task_handle * const task_handle)
{
    ...
    #if OSAL_TRACE_STACK
    creating_task_id = task_id;
    creating_task_depth = stack_depth;
    #endif
...
#if OSAL_TRACE_STACK
int rtos_task_stack_check(rtos_task_handle task_handle)
{
    for (int idx = 0; idx < MAX_TASK ; idx++) {
        if (task_stack_table[idx].handle == task_handle)
            return task_stack_table[idx].depth;
    }
    return 0;
}
#endif
...
void rtos_task_info(void)
{
    ...
    if (task_status_array) {
        ...
        #else
        dbg("TaskName\tNum\tPrio\tStat\tStkHWM\tHdl\tStkBA\tStkDth\n");
        for (idx = 0; idx < array_size; idx++) {
            dbg("%s%s%d\t%d\t%c\t%d\t%x\t%x\t%d\n",
                task_status_array[idx].pcTaskName,
                (strlen(task_status_array[idx].pcTaskName) < (configMAX_TASK_NAME_LEN - 8)) ? "\t\t" : "\t", task_status_array[idx].xTaskNumber,
                task_status_array[idx].uxCurrentPriority, task_st_ch[task_status_array[idx].eCurrentState],
                task_status_array[idx].usStackHighWaterMark, (uint32_t)task_status_array[idx].xHandle, (uint32_t)task_status_array[idx].pxStackBase,
                rtos_task_stack_check(task_status_array[idx].xHandle));
        }
        #endif
        if (TASK_NUM_TRACED_MAX < array_size) {
...
int rtos_init(void)
{
    ...
    #if OSAL_TRACE_STACK
    memset(task_stack_table, 0, sizeof(task_stack_table));
    #endif
...
void rtos_trace_task(int id, void *task)
{
    ...
    #if OSAL_TRACE_STACK
    if (id == RTOS_TRACE_CREATE) {
        if ((creating_task_id == UNDEF_TASK) &&
            (task_stack_table[IDLE_TASK].handle == NULL)
            #if( configSUPPORT_STATIC_ALLOCATION == 0 )
            && (task == xTaskGetIdleTaskHandle())
            #endif
            )
        {
            task_stack_table[IDLE_TASK].handle = task;
            task_stack_table[IDLE_TASK].depth = configMINIMAL_STACK_SIZE;
            creating_task_id = IDLE_TASK;
        }
        else if ((creating_task_id == UNDEF_TASK) &&
            (task_stack_table[TMR_DAEMON_TASK].handle == NULL)
            #if( configSUPPORT_STATIC_ALLOCATION == 0 )
            && (task == xTimerGetTimerDaemonTaskHandle())
            #endif
            )
        {
            task_stack_table[TMR_DAEMON_TASK].handle = task;
            task_stack_table[TMR_DAEMON_TASK].depth = configTIMER_TASK_STACK_DEPTH;
            creating_task_id = TMR_DAEMON_TASK;
        }
        else if (creating_task_id < MAX_TASK)
        {
            task_stack_table[creating_task_id].handle = task;
            task_stack_table[creating_task_id].depth = creating_task_depth;
        }

        //dbg("Create task %d,%d\n", creating_task_id, creating_task_depth);
        creating_task_id = UNDEF_TASK;
        creating_task_depth = 0;
    }
    #endif
