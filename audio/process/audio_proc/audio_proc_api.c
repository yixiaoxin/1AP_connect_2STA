...
void audio_proc_i2s_mclk_config(void)
{
    ...
    if(hclk == 0) {
    dbg("HCLK not ready! Defer MCLK config");
    return; // 安全退出
        }
        dbg("hclk=%d\n",hclk);
...
{
#if PLF_HW_ASIC
    // PinName mclk_pin = audio_config_i2s_mclk_pin_get();
    PinName mclk_pin = PA_11;
    //MCLK
    #if PLF_AIC8800
    if (mclk_pin == PA_15) {
        iomux_gpio_config_sel_setf(15, 8);
    } else if (mclk_pin == PA_4) {
    ...
    #else
    AIC_CPUSYSCTRL->cpu_sys_tports_sel = ((AIC_CPUSYSCTRL->cpu_sys_tports_sel) & (~(CPU_SYS_CTRL_CFG_DBG_CLK_SEL_MASK))) | CPU_SYS_CTRL_CFG_DBG_CLK_SEL(9);
    if (mclk_pin == PA_11) {
        iomux_gpioa_config_sel_setf(11, 5);
    } else if (mclk_pin == PA_5) {
        iomux_gpioa_config_sel_setf(5, 6);
    } else if (mclk_pin == PA_2) {
        iomux_gpioa_config_sel_setf(2, 5);
    }
    #endif
    #endif
#endif
...
void audio_proc_i2s0_iomux_config(void)
{
    ...
    /*
     * AIC8800M40B I2S0 capture only:
     *
     * GPIOA0 -> I2S_LRCK_0
     * GPIOA1 -> I2S_BCK_0
     * GPIOA2 -> I2S_DAT_IN_0
     *
     * 不配置 GPIOA3 -> I2S_DAT_OUT_0。
     * 当前只采 Mic，不需要 DOUT。
     */
    iomux_gpioa_config_sel_setf(0, func_sel);
    iomux_gpioa_config_sel_setf(1, func_sel);
    ...
    iomux_gpioa_config_pull_up_setf(0, 0);
    iomux_gpioa_config_pull_dn_setf(0, 0);
    iomux_gpioa_config_pull_frc_setf(0, 0);

    iomux_gpioa_config_pull_up_setf(1, 0);
    iomux_gpioa_config_pull_dn_setf(1, 0);
    iomux_gpioa_config_pull_frc_setf(1, 0);

    iomux_gpioa_config_pull_up_setf(2, 0);
    iomux_gpioa_config_pull_dn_setf(2, 0);
    iomux_gpioa_config_pull_frc_setf(2, 0);
#endif
}
// -------------------------------------------------------------------------------
