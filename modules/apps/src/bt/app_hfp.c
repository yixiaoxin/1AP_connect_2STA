...
void app_hfp_msg_handle(AIC_EVENT *event)
{
    ...
    {
        case AIC_ADP_HFP_CONNECTED:
            TRACE("APP:app_hfp_connect , result = %d ,role = %d\n",aic_hfp_msg->p.conP.reason,aic_hfp_msg->role);
...
#endif
#if APP_SUPPORT_HFG
                    app_hfg_set_def_device(aic_hfp_msg->bdaddr);
#endif
                }
            }
        ...
        case AIC_ADP_HFP_DISCONNECTED:
            {
                ...
                TRACE("APP:app_hfp_disconnect , reason = %d,role = %d\n",discReason,aic_hfp_msg->role);
