...
int app_bt_queue_process(APP_BT_MSG* msg)
{
    ...
    switch (msg->msg_id) {
        ...
        case BT_HFG_SET_VGM:
            {
                uint8_t vol = (uint8_t)msg->param.p.key;
                app_hfg_set_vgm(vol);
            }
            break;
        case BT_HFG_SET_VGS:
...
int app_bt_hfg_set_vgm(uint8_t vol)
{
#if APP_SUPPORT_HFG == 1
    APP_BT_MSG msg;
    msg.msg_id = BT_HFG_SET_VGM;
    msg.param.p.key = vol;
    app_bt_queue_send(&msg);
#endif
    return 0;
...
int app_bt_hfg_set_vgs(uint8_t vol)
{
#if APP_SUPPORT_HFG == 1
    APP_BT_MSG msg;
    msg.msg_id = BT_HFG_SET_VGS;
    msg.param.p.key = vol;
    app_bt_queue_send(&msg);
#endif
    return 0;
