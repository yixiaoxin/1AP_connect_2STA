        ...
        case AIC_ADP_INQUIRY_RESULT:
            {
                ...
                if(aic_mgr_msg->p.inqResultP.inqMode == 2){
                    ...
                    #if APP_SUPPORT_HFG
                    char peer_name[] = "earphone";
                    if(!memcmp(peer_name,name,sizeof(peer_name)) && aic_mgr_msg->p.inqResultP.rssi > -50){
                        app_bt_inquiry_cancel();
                        app_hfg_set_def_device(aic_mgr_msg->bdaddr);
