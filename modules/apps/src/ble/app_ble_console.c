...
static int app_ble_adv_param_update(int argc, char * const argv[])
{
    ...
    max_tx_pwr = console_cmd_strtoul(argv[3], NULL, 10);
    dbg("%s adv_intv_min %d,adv_intv_max %d,max_tx_pwr %d,%d\r\n", __func__,adv_intv_min,adv_intv_max,max_tx_pwr,(int8_t)max_tx_pwr);

    app_ble_adv_param_update_msg_send(adv_intv_min,adv_intv_max,max_tx_pwr);
...
{
    unsigned int dut_use_uart = 0;

    if (argc < 2) {
        return ERR_WRONG_ARGS;
    }

    dut_use_uart = console_cmd_strtoul(argv[1], NULL, 0);
    if(dut_use_uart)
        bt_drv_enter_uart_dut_test();
    else
...
static int app_ble_scan_en(int argc, char * const argv[])
{
    ...
    scan_en = console_cmd_strtoul(argv[1], NULL, 10);
    if (scan_en > 0x03) {
        return ERR_WRONG_ARGS;
    }
...
static int app_ble_send_phy_update(int argc, char * const argv[])
{
    unsigned int conidx = 0;
...
static int app_ble_bt_test_en(int argc, char * const argv[])
{
    unsigned int tgl = 0;
    ...
    tgl = console_cmd_strtoul(argv[1], NULL, 10);
    dbg("%s, %d\n",__func__,tgl);
#if !PLF_M2D_BLE
    if (tgl) {
        bt_test_start_server();
    } else {
        bt_test_stop_server();
    }
#endif
...
extern int bt_test_parse_cmd_line(int argc, char * const argv[]);
...
void app_ble_test_command_add(void)
{
    ...
    console_cmd_add("ble_adv_param", "eg:ble_adv_param <adv_intv_min> <adv_intv_max> <max_tx_power>", 1, 4, app_ble_adv_param_update);
    ...
    console_cmd_add("dut", "enter dut   eg: dut 1/0 : 1 use uart1; 0 enable scan and wait", 1, 2, app_ble_enable_dut);
    ...
    console_cmd_add("ble_updphy", "ble send phy update   eg: ble_updphy <conidx> ", 1, 2, app_ble_send_phy_update);
    ...
    console_cmd_add("bt_test_en", "bt_test -s  eg: bt_test_en <1/0> ", 1, 2, app_ble_bt_test_en);
#if !PLF_M2D_BLE
    console_cmd_add("bt_test", "bt_test -c  eg: bt_test <param> .... ", 1, 200, bt_test_parse_cmd_line);
#endif
}
#endif // (PLF_CONSOLE)
