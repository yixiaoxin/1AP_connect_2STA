...
int uart_printf(const char *format, va_list arg)
{
    ...
    if (size > 0) {
        /*
         * vsnprintf() returns the length that would have been written when
         * the output is truncated.  The UART loop below must only read bytes
         * that actually exist in buffer[], otherwise an overlong log line can
         * print memory garbage after the valid text.
         */
