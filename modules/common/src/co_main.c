...
#endif
static bool dut_task_suspended;
static rtos_task_handle dut_notify_handle = 0;

void dut_task_notify_handle_register(rtos_task_handle handle)
{
    dut_notify_handle = handle;
}

uint32_t dut_task_suspend(void)
{
    uint32_t retvalue = 0;
    GLOBAL_INT_DISABLE();
    dut_task_suspended = true;
    GLOBAL_INT_RESTORE();
...
void dut_task_resume(bool isr,uint32_t value)
{
    //if (!dut_task_suspended) {
        //return;
    //}
    dut_task_suspended = false;
    //dbg("ble_task_handle = 0x%x\n",ble_task_handle);

    // Notify the bt task
    if (dut_notify_handle != 0) {
        rtos_task_notify_setvalue(dut_notify_handle, value, isr);
    }
}
