...
#if defined(CONFIG_WPA_USING_NET_FD_SET)
#include "net_al.h"
#endif
				     ...
				     fd_set *fds)
{
	...
	for (i = 0; i < table->count; i++) {
        ...
        #if defined(CONFIG_WPA_USING_NET_FD_SET)
        net_fd_set(table->table[i].sock, fds);
        #else
				      ...
				      fd_set *fds)
{
	...
	for (i = 0; i < table->count; i++) {
        int is_set;
        #if defined(CONFIG_WPA_USING_NET_FD_SET)
        is_set = net_fd_isset(table->table[i].sock, fds);
        #else
        is_set = FD_ISSET(table->table[i].sock, fds);
        #endif
		if (is_set) {
			table->table[i].handler(table->table[i].sock,
						table->table[i].eloop_data,
...
void eloop_wait_for_read_sock(int sock)
{
    ...
    net_fd_zero(p_rfds);
    net_fd_set(sock, p_rfds);
    select(sock + 1, p_rfds, NULL, NULL, NULL);
    net_fd_free(p_rfds);
    #else
	FD_ZERO(&rfds);
	FD_SET(sock, &rfds);
