	...
	*ssid_len = P2P_WILDCARD_SSID_LEN + 2 + p2p->cfg->ssid_postfix_len;
    ...
    if (p2p->cfg->ssid_postfix && p2p->cfg->ssid_postfix_len > 0) {
        ...
        *ssid_len =  p2p->cfg->ssid_postfix_len;
        ...
        os_memcpy(ssid, "DIRECT-AIC-P2P-IOT", 18);
        *ssid_len = 18;
    }
    //dbg("p2p->cfg->ssid_postfix %s, ssid %s\n", p2p->cfg->ssid_postfix, ssid);
    #endif
}
...
#define P2P_PRINT_LOCAL_BUF_SIZE    256

void p2p_dbg(struct p2p_data *p2p, const char *fmt, ...)
{
	va_list ap;
	char buf[P2P_PRINT_LOCAL_BUF_SIZE];
