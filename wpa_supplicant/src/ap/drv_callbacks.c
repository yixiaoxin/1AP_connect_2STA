...
#ifdef CFG_HOSTAPD
#include "hostapd_eloop.h"
#define eloop_is_timeout_registered hostapd_eloop_is_timeout_registered
#define eloop_register_timeout hostapd_eloop_register_timeout
#define eloop_cancel_timeout hostapd_eloop_cancel_timeout
#define eloop_terminate hostapd_eloop_terminate
#define eloop_replenish_timeout hostapd_eloop_replenish_timeout
#define eloop_deplete_timeout hostapd_eloop_deplete_timeout
#endif /* CFG_HOSTAPD */
...
#ifdef CONFIG_RWNX
    //dbg("%s %d\r\n", __func__, hapd->iface->conf->hw_mode);
    if ((hapd->iface->conf->hw_mode != HOSTAPD_MODE_IEEE80211A) && \
        (freq > 5000)) {
        hapd->iface->conf->hw_mode = HOSTAPD_MODE_IEEE80211A;
    } else if ((hapd->iface->conf->hw_mode != HOSTAPD_MODE_IEEE80211G) && \
        (freq < 5000)) {
        hapd->iface->conf->hw_mode = HOSTAPD_MODE_IEEE80211G;
    } 
...
#else
void rwnx_hostapd_wpa_supplicant_event(void *ctx, enum wpa_event_type event,
			  union wpa_event_data *data)
{
	struct hostapd_data *hapd = ctx;
#ifndef CONFIG_NO_STDOUT_DEBUG
...
void rwnx_hostapd_wpa_supplicant_event_global(void *ctx, enum wpa_event_type event,
				 union wpa_event_data *data)
{
	struct hapd_interfaces *interfaces = ctx;
	struct hostapd_data *hapd;
	...
	if (hapd)
		rwnx_hostapd_wpa_supplicant_event(hapd, event, data);
}
