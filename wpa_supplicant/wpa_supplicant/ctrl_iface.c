					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
	...
	if (os_strcasecmp(cmd, "EAPOL::heldPeriod") == 0) {
		...
		wpa_s->conf->country[0] = cmd[8];
		wpa_s->conf->country[1] = cmd[9];
...
#ifdef CONFIG_ACS
int hostapd_config_parse_acs_chan_bias(struct hostapd_config *conf,
					      char *pos)
{
    ...
    #else
		tmp = os_realloc_array(bias, 3, sizeof(*bias));
		if (!tmp)
			goto fail;
		bias = tmp;
        bias[0].channel = 1;
