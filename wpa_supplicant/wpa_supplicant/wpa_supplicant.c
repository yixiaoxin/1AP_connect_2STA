...
#if (PLF_AIC8800)
static struct part_of_wpa_supplicant part_wpas;
PRIVATE_HOST_DECLARE(G1WLAN, struct part_of_wpa_supplicant, part_wpas);

void wpa_sm_data_set(struct wpa_sm *sm)
{
	struct wpa_supplicant *ws = (struct wpa_supplicant *)(sm->ctx->ctx);
	part_wpas.wpa_state = ws->wpa_state;
	part_wpas.drv_flags = ws->drv_flags;
	memcpy(part_wpas.bssid, ws->bssid, ETH_ALEN);
	memcpy(part_wpas.ifname, ws->ifname, 10);
    memcpy(&part_wpas.ssid, ws->conf->ssid, sizeof(struct wpa_ssid));
    memcpy(part_wpas.ssid_str, ws->conf->ssid->ssid, ws->conf->ssid->ssid_len);

    if (ws->eapol_received) {
    	part_wpas.eapol_received = ws->eapol_received;
    	part_wpas.key_mgmt  = ws->key_mgmt;

    	part_wpas.pairwise_cipher = sm->pairwise_cipher;
    	part_wpas.msg_3_of_4_ok = sm->msg_3_of_4_ok;
    	part_wpas.ptk_set  = sm->ptk_set;
    	part_wpas.proto    = sm->proto;
    	part_wpas.group_cipher = sm->group_cipher;
    	memcpy(&part_wpas.wpa_ptk_t, &(sm->ptk), sizeof(struct wpa_ptk));
    }
}

void wpa_supplicant_data_set(struct wpa_supplicant *ws)
{
	part_wpas.wpa_state = ws->wpa_state;
	part_wpas.drv_flags = ws->drv_flags;
	memcpy(part_wpas.bssid, ws->bssid, ETH_ALEN);
...
void wpa_supplicant_data_restore(struct wpa_supplicant *ws)
{
	ws->wpa_state = part_wpas.wpa_state;
	ws->drv_flags = part_wpas.drv_flags;
	ws->key_mgmt  = part_wpas.key_mgmt;
	memcpy(ws->bssid, part_wpas.bssid, ETH_ALEN);
	memcpy(ws->ifname, part_wpas.ifname, 10);
    if (part_wpas.ssid.ssid) {
        ws->conf->ssid = os_malloc(sizeof(struct wpa_ssid));
        //dbg(" %s %x\r\n", __func__, ws->conf->ssid);
        memcpy(ws->conf->ssid,  &part_wpas.ssid, sizeof(struct wpa_ssid));
        ws->conf->ssid->ssid = (u8 *)os_malloc(32);
        //dbg(" %s %x\r\n", __func__, ws->conf->ssid->ssid);
        memcpy(ws->conf->ssid->ssid, part_wpas.ssid_str, 31);
        dl_list_init(&ws->conf->ssid->psk_list);
        ws->conf->ssid->passphrase = NULL;
    }
    {
        ws->conf->ssid->passphrase = NULL;
        ws->conf->ssid->ext_psk = NULL;
        ws->conf->ssid->sae_password = NULL;
        ws->conf->ssid->sae_password_id = NULL;
        ws->conf->ssid->id_str = NULL;
        ws->conf->ssid->scan_freq = NULL;
        ws->conf->ssid->freq_list = NULL;
        ws->conf->ssid->bgscan = NULL;
        ws->conf->ssid->p2p_client_list = NULL;
        ws->conf->ssid->bssid_blacklist = NULL;
        ws->conf->ssid->bssid_whitelist = NULL;
        ws->conf->ssid->dpp_connector = NULL;
        ws->conf->ssid->dpp_netaccesskey = NULL;
        ws->conf->ssid->dpp_csign = NULL;

    }
    ws->conf->pssid = os_realloc_array(ws->conf->pssid, 1, sizeof(struct wpa_ssid *));
    //ws->conf->pssid[0] = os_malloc(sizeof(struct wpa_ssid));
    //memcpy(ws->conf->pssid[0],  &part_wpas.ssid, sizeof(struct wpa_ssid));
    //dl_list_init(&ws->conf->pssid[0]->psk_list);
    //ws->conf->pssid[0]->ssid = (u8 *)os_malloc(32);
    //memcpy(ws->conf->pssid[0]->ssid, part_wpas.ssid_str, 31);
    ws->conf->pssid[0] = ws->conf->ssid;
    ws->conf->num_prio = 1;

    if (part_wpas.eapol_received) {
        memcpy(ws->wpa->bssid, part_wpas.bssid, ETH_ALEN);
    	ws->eapol_received = part_wpas.eapol_received;
    	ws->wpa->msg_3_of_4_ok = part_wpas.msg_3_of_4_ok;
    	ws->wpa->ptk_set = part_wpas.ptk_set;
    	ws->wpa->proto   = part_wpas.proto;
    	ws->wpa->group_cipher = part_wpas.group_cipher;
    	ws->wpa->key_mgmt = part_wpas.key_mgmt;
    	ws->wpa->pairwise_cipher = part_wpas.pairwise_cipher;
    	memcpy(&(ws->wpa->ptk), &part_wpas.wpa_ptk_t, sizeof(struct wpa_ptk));
    }
}

void wpas_data_save(void)
{
	backup_part_wpas = part_wpas;
}

void wpas_data_restore(void)
{
	part_wpas = backup_part_wpas;
}
#endif
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	wpa_s->prev_scan_wildcard = 1; // scan for specific SSID at first time, modified @ 2024-05-18
...
#if defined(CONFIG_AP) || defined(CFG_HOSTAPD)
	eap_server_unregister_methods();
#endif /* CONFIG_AP || CFG_HOSTAPD */
