...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
    ...
    #else /* CONFIG_RWNX */
	if (!(fhost_usr_cfg.wifi_ext_flags & WIFI_EXT_NO_WILDCARD_SCAN_FLAG) && wpa_s->prev_scan_ssid != WILDCARD_SSID_SCAN) {
		while (ssid) {
			if (ssid == wpa_s->prev_scan_ssid) {
				ssid = ssid->next;
				break;
	...
	if (ssid && max_ssids == 1) {
        ...
        #else /* CONFIG_RWNX */
		if (!(fhost_usr_cfg.wifi_ext_flags & WIFI_EXT_NO_WILDCARD_SCAN_FLAG) && !wpa_s->prev_scan_wildcard) {
			params.ssids[0].ssid = NULL;
			params.ssids[0].ssid_len = 0;
			wpa_s->prev_scan_wildcard = 1;
			wpa_dbg(wpa_s, MSG_DEBUG, "Starting AP scan for "
