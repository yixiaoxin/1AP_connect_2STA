...
struct fhost_frame_info
{
    ...
    uint32_t rate;
    /**
     * Received Format (NON-HT/HT/VHT/HE)
     */
    uint8_t format;
...
#define WIFI_EXT_SKIP_WIFI_PROBE_CARD_EN_FLAG   (0x01UL << 8)
#define WIFI_EXT_TX_TIMEOUT_DROP_PKT_EN_FLAG    (0x01UL << 9)
#define WIFI_EXT_PARSE_RX_VECT_EN_FLAG          (0x01UL << 10) // enable parsing rx vect
...
#endif
    uint8_t *ap_vendor_specific;
    struct {
        uint8_t  wait_to_ms; // wait time once, total_time = wait_to_ms * wait_to_cnt2
        uint8_t  wait_to_cnt1;
        uint16_t wait_to_cnt2;
    } txlist_sema_cfg;
} fhost_user_cfg_t;
...
/**
 * Function prototype for scan callbacks
 */
typedef void (*cb_fhost_scan)(struct scanu_result_ind *info);

/**
 * Example:
 *  void fhost_console_scan_cb(struct scanu_result_ind *info)
 *  {
 *      dbg("Freq %d %d\r\n", info->center_freq, info->rssi);
 *  }
 *  fhost_set_scan_cb(fhost_console_scan_cb);
 */
void fhost_set_scan_cb(cb_fhost_scan cb);
