...
#if (PLF_AIC8800M40)
void sta_rx_stat_set(uint8_t *addr, struct rx_vector *vect)
{
    ...
    if (RWNX_VIF_TYPE(fhost_vif) == VIF_MONITOR) {
        ...
        // printk("%s: AP mode\n", __func__);
        sta = vif_mgmt_get_sta_by_addr(addr);
    }
    // printk("Addr %02x:%02x:%02x:%02x:%02x:%02x, sta[%x]\r\n", addr[0], addr[1], addr[2], addr[3], addr[4], addr[5], sta);
    if (sta) {
        memcpy(&sta->stats.last_rx, vect, sizeof(struct rx_vector));
    }
}
#endif
#endif /* PLF_RSSI_DATAPKT */
...
void rwnx_timer_init(void)
{
    ...
    ret = rtos_task_create(rwnx_timer_task, "rwnx_timer_task",
                           REORDER_TIMER_TASK, TASK_STACK_SIZE_REORDER_TIMER,
                           (void*)timer_env, TASK_PRIORITY_REORDER_TIMER,
...
/*
 * SoftAP RX reorder v5.3.3 (RXBYP1).
 *
 * The v5.3.0 guard protected heap by globally bypassing reorder for 200 ms.
 * That avoided allocation failure, but a burst of unordered frames could then
 * reach TCP and create a periodic recover/congest/recover cycle.
 *
 * v5.3.3 keeps the v5.3.2 bounded allocation/sequence semantics and changes
 * only the pressure fallback used by the realtime TCP audio path.  When a
 * genuine sequence hole cannot be cached because a per-TID/global/heap bound
 * is reached, forward that one original MSDU to the IP stack instead of hard
 * dropping it.  TCP can reorder one out-of-order segment; an application-level
 * hard drop always forces retransmission and was visible as 10 ms audio gaps.
 * There is still no timed/global bypass mode.
 *
 * The preserved v5.3.2 sequence semantics are:
 *   - an in-order frame uses the original host RX buffer (no malloc/copy),
 *   - a frame with a sequence number older than ind_sn is forwarded instead
 *     of dropped because multiple A-MSDU subframes can share one 802.11 SN,
 *   - multiple queued subframes with the same SN are retained in arrival order,
 *   - only a genuine sequence hole allocates a copied reorder frame,
 *   - queued copies remain bounded and pressure handling stays per STA/TID.
 */
static volatile uint16_t s_reord_queued_frames;
static volatile uint16_t s_reord_peak_frames;
static volatile uint32_t s_reord_fast_path;
static volatile uint32_t s_reord_gap_cached;
static volatile uint32_t s_reord_force_release;
static volatile uint32_t s_reord_pressure_drop;
static volatile uint32_t s_reord_pressure_bypass;
static volatile uint32_t s_reord_dup_drop;
static volatile uint32_t s_reord_old_forward;
static volatile uint32_t s_reord_same_seq;
static volatile uint32_t s_reord_alloc_fail;
static volatile uint32_t s_reord_low_heap;
static volatile uint32_t s_reord_limit;
static uint32_t s_reord_last_report_ms;

/* NETDIAG1: read-only snapshot for the AP audio task.  Keep these counters
 * diagnostic-only: no reorder thresholds, timers, forwarding or allocation
 * behavior is changed by this API. */
void rwnx_reord_v533_diag_snapshot(uint16_t *queued, uint16_t *peak,
                                   uint32_t *cache, uint32_t *force,
                                   uint32_t *bypass, uint32_t *old,
                                   uint32_t *low, uint32_t *limit)
{
    if (queued) *queued = s_reord_queued_frames;
    if (peak) *peak = s_reord_peak_frames;
    if (cache) *cache = s_reord_gap_cached;
    if (force) *force = s_reord_force_release;
    if (bypass) *bypass = s_reord_pressure_bypass;
    if (old) *old = s_reord_old_forward;
    if (low) *low = s_reord_low_heap;
    if (limit) *limit = s_reord_limit;
}

static int reord_heap_free(void)
{
    int total, free_heap, min_heap;
    rtos_heap_info(&total, &free_heap, &min_heap);
    (void)total;
    (void)min_heap;
    return free_heap;
}

static void reord_stats_report(uint32_t now, int heap_free,
                               const char *reason)
{
    if ((uint32_t)(now - s_reord_last_report_ms) < 1000U) {
        return;
    }
    s_reord_last_report_ms = now;
    dbg("REORD_V533 reason=%s heap=%d queued=%u peak=%u fast=%u cache=%u force=%u drop=%u bypass=%u dup=%u old=%u same=%u alloc=%u low=%u limit=%u t=%u\n",
        reason, heap_free,
        (unsigned)s_reord_queued_frames,
        (unsigned)s_reord_peak_frames,
        (unsigned)s_reord_fast_path,
        (unsigned)s_reord_gap_cached,
        (unsigned)s_reord_force_release,
        (unsigned)s_reord_pressure_drop,
        (unsigned)s_reord_pressure_bypass,
        (unsigned)s_reord_dup_drop,
        (unsigned)s_reord_old_forward,
        (unsigned)s_reord_same_seq,
        (unsigned)s_reord_alloc_fail,
        (unsigned)s_reord_low_heap,
        (unsigned)s_reord_limit,
        (unsigned)now);
}

static int reord_original_ind(struct fhost_rx_buf_tag *buf, uint8_t forward)
{
    if (forward) {
...
static int reord_original_drop(struct fhost_rx_buf_tag *buf)
{
    fhost_rx_buf_push(buf);
    return 0;
}

static void reord_queued_dec(void)
{
    if (s_reord_queued_frames > 0U) {
        s_reord_queued_frames--;
    }
}
...
struct recv_msdu *reord_rxframe_alloc(struct fhost_rx_buf_tag *buf, uint16_t seq_num, uint8_t tid, uint8_t forward)
{
    ...
    s_reord_queued_frames++;
    if (s_reord_queued_frames > s_reord_peak_frames) {
        s_reord_peak_frames = s_reord_queued_frames;
    }
    s_reord_gap_cached++;
    fhost_rx_buf_push(buf);
    return rxframe;
...
int reord_rxframe_enqueue(struct reord_ctrl *preorder_ctrl, struct recv_msdu *prframe)
{
    ...
    while (plframe) {
        ...
        if (SN_LESS(plframe->seq_num, prframe->seq_num)) {
            ...
            /* Keep all de-aggregated A-MSDU subframes that share one SN.
             * Insert the new MSDU after the existing same-SN run. */
            plframe = (struct recv_msdu *)co_list_next(&plframe->hdr);
            continue;
        } else {
            break;
...
/* Caller holds preorder_ctrl->reord_list_lock.  Skip only the
 * missing sequence immediately before the first queued run, then release that
 * contiguous run.  Keep the control enabled and retain the resulting ind_sn. */
static int reord_force_advance_locked(struct reord_ctrl *preorder_ctrl)
{
    struct recv_msdu *prframe;
    int released = 0;
    ...
    while (1) {
        prframe = (struct recv_msdu *)co_list_pick(&preorder_ctrl->reord_list);
        if (prframe == NULL ||
            SN_LESS(preorder_ctrl->ind_sn, prframe->seq_num)) {
            break;
        }

        co_list_pop_front(&preorder_ctrl->reord_list);
        if (SN_EQUAL(preorder_ctrl->ind_sn, prframe->seq_num)) {
            preorder_ctrl->ind_sn =
                (preorder_ctrl->ind_sn + 1U) & 0x0FFFU;
        }
        reord_single_frame_ind(prframe);
        if (preorder_ctrl->list_cnt > 0U) {
            preorder_ctrl->list_cnt--;
        }
        released++;
    }
...
static void reord_timer_sync(struct reord_ctrl *preorder_ctrl)
{
    bool pending;

    rtos_mutex_lock(preorder_ctrl->reord_list_lock, -1);
    pending = !co_list_is_empty(&preorder_ctrl->reord_list);
    rtos_mutex_unlock(preorder_ctrl->reord_list_lock);
    ...
    if (pending) {
        ...
        if (preorder_ctrl->reord_timer) {
            if (!rwnx_timer_is_posted(preorder_ctrl->reord_timer)) {
                rwnx_timer_start(preorder_ctrl->reord_timer,
                                 REORDER_UPDATE_TIME, true);
            }
        } else {
            rwnx_timer_create(&preorder_ctrl->reord_timer,
                              REORDER_UPDATE_TIME, preorder_ctrl,
                              reord_timeout_handler, true, false);
        }
#else
        if (preorder_ctrl->reord_timer) {
            rtos_timer_stop(preorder_ctrl->reord_timer, 0);
        ...
        preorder_ctrl->reord_timer = rtos_timer_create("reord_timer",
                                      REORDER_UPDATE_TIME, 1, NULL,
                                      reord_timeout_handler);
#endif
    } else if (preorder_ctrl->reord_timer) {
#if (AICWF_RWNX_TIMER_EN)
        if (rwnx_timer_is_posted(preorder_ctrl->reord_timer)) {
...
/* Re-evaluate the original frame after a forced advance.  The caller holds
 * the TID lock.  Return 1 for in-order delivery, -1 for old/duplicate, and 0
 * when a sequence hole still remains. */
static int reord_after_advance_locked(struct reord_ctrl *preorder_ctrl,
                                      uint16_t seq_num)
{
    if (SN_LESS(seq_num, preorder_ctrl->ind_sn)) {
        return -1;
...
int reord_process_unit(struct fhost_rx_buf_tag *buf, uint16_t seq_num,
                       uint8_t *macaddr, uint8_t tid, uint8_t forward)
{
    int ret;
    int heap_free = -1;
    int released;
    ...
    uint32_t now = rtos_now(0);
    struct recv_msdu *pframe;
    struct reord_ctrl *preorder_ctrl = NULL;
    struct reord_ctrl_info *reord_info = NULL;

    /* Find or create the small per-STA control first.  Do not copy the RX
     * frame until a real sequence hole has been confirmed. */
    rtos_mutex_lock(stas_reord_lock, -1);
    reord_info = (struct reord_ctrl_info *)co_list_pick(&stas_reord_list);
    while (reord_info) {
        ...
        reord_info =
            (struct reord_ctrl_info *)co_list_next(&reord_info->hdr);
    }
    if (preorder_ctrl == NULL) {
        ...
        if (!reord_info) {
            rtos_mutex_unlock(stas_reord_lock);
            s_reord_alloc_fail++;
            s_reord_pressure_bypass++;
            heap_free = reord_heap_free();
            reord_stats_report(now, heap_free, "ctrl_alloc_bypass");
            return reord_original_ind(buf, forward);
        }
        co_list_push_back(&stas_reord_list, &reord_info->hdr);
        preorder_ctrl = &reord_info->preorder_ctrl[tid];
    } else if (preorder_ctrl->enable == false) {
        preorder_ctrl->enable = true;
        preorder_ctrl->ind_sn = 0xFFFFU;
        preorder_ctrl->wsize_b = AICWF_REORDER_WINSIZE;
    }
    rtos_mutex_unlock(stas_reord_lock);
    ...
    /* First/in-order frame: zero-copy direct delivery. */
    if (preorder_ctrl->ind_sn == 0xFFFFU &&
        co_list_is_empty(&preorder_ctrl->reord_list)) {
        preorder_ctrl->ind_sn = (seq_num + 1U) & 0x0FFFU;
        rtos_mutex_unlock(preorder_ctrl->reord_list_lock);
        s_reord_fast_path++;
        return reord_original_ind(buf, forward);
    }

    /* A sequence older than ind_sn is not necessarily a duplicate.  The
     * firmware can deliver multiple de-aggregated A-MSDU subframes with the
     * same 802.11 sequence number.  Preserve vendor behavior and forward the
     * original MSDU; TCP removes a true retransmitted duplicate safely. */
    if (SN_LESS(seq_num, preorder_ctrl->ind_sn)) {
        rtos_mutex_unlock(preorder_ctrl->reord_list_lock);
        s_reord_old_forward++;
        return reord_original_ind(buf, forward);
    ...
    if (SN_EQUAL(seq_num, preorder_ctrl->ind_sn)) {
        ...
        rtos_mutex_unlock(preorder_ctrl->reord_list_lock);
        s_reord_fast_path++;
        reord_original_ind(buf, forward);
        reord_rxframes_ind(preorder_ctrl);
        reord_timer_sync(preorder_ctrl);
        return 0;
    }

    /* Multiple de-aggregated A-MSDU subframes may be waiting with the same
     * 802.11 sequence number.  Count that case but keep every MSDU. */
    {
        struct recv_msdu *queued =
            (struct recv_msdu *)co_list_pick(&preorder_ctrl->reord_list);
        while (queued) {
            if (SN_EQUAL(seq_num, queued->seq_num)) {
                s_reord_same_seq++;
                break;
            }
            if (SN_LESS(seq_num, queued->seq_num)) {
                break;
            }
            queued = (struct recv_msdu *)co_list_next(&queued->hdr);
        }
    }

    /* A real sequence hole exists.  Keep only a few copied frames.  When a
     * bound is reached, advance this TID once; never enter a global bypass. */
    if (preorder_ctrl->list_cnt >= AICWF_REORDER_MAX_HOLD ||
        s_reord_queued_frames >= AICWF_REORDER_GLOBAL_MAX_HOLD) {
        limit_reason =
            (preorder_ctrl->list_cnt >= AICWF_REORDER_MAX_HOLD) ?
            "tid_limit" : "global_limit";
        released = reord_force_advance_locked(preorder_ctrl);
        s_reord_force_release += (uint32_t)released;
        s_reord_limit++;
        after = reord_after_advance_locked(preorder_ctrl, seq_num);
        rtos_mutex_unlock(preorder_ctrl->reord_list_lock);
        ...
        reord_timer_sync(preorder_ctrl);
        reord_stats_report(now, heap_free, limit_reason);
        return 0;
    }
    rtos_mutex_unlock(preorder_ctrl->reord_list_lock);

    /* Heap is checked only before a genuine out-of-order copy, not for every
     * normal packet. */
    heap_free = reord_heap_free();
    if (heap_free < (int)AICWF_REORDER_LOW_HEAP_BYTES) {
        rtos_mutex_lock(preorder_ctrl->reord_list_lock, -1);
        released = reord_force_advance_locked(preorder_ctrl);
        s_reord_force_release += (uint32_t)released;
        s_reord_low_heap++;
        ...
        reord_timer_sync(preorder_ctrl);
        reord_stats_report(now, heap_free, "low_heap_bypass");
        return 0;
    }
    ...
    if (pframe == NULL) {
        ...
        reord_timer_sync(preorder_ctrl);
        reord_stats_report(now, heap_free, "frame_alloc_bypass");
        return 0;
    }
    ...
    ret = reord_need_check(preorder_ctrl, pframe->seq_num);
    if (ret) {
        /* Match the vendor path: an older SN can be another A-MSDU MSDU.
         * The original buffer was already returned, so indicate the copy. */
...
{
    int ret;
    dbg("REORD_V533 init low=%u maxhold=%u global=%u timeout=%u winsize=%u zero_copy=1 amsdu_safe=1 pressure_bypass=single_frame global_bypass=0\n",
        (unsigned)AICWF_REORDER_LOW_HEAP_BYTES,
        (unsigned)AICWF_REORDER_MAX_HOLD,
        (unsigned)AICWF_REORDER_GLOBAL_MAX_HOLD,
        (unsigned)REORDER_UPDATE_TIME,
        (unsigned)AICWF_REORDER_WINSIZE);
...
uint8_t rwnx_rxdataind(void *pthis, void *hostid)
{
    ...
    #if CONFIG_RX_PKT_SNR
    #if PLF_AIC8800M40
    if (((machdr_ptr->fctl & 0xF) == MAC_FCTRL_DATA_T) && ((int8_t)(rxhdr->rx_vect2.evm1) > 0)) {
        data_pkt_snr = (int8_t)(rxhdr->rx_vect2.evm1);
        //dbg("fctl=%4x, rssi=%3d, snr=%3d\n", machdr_ptr->fctl, rssi_saved, data_pkt_snr);
    }
    #endif
    #endif
    ...
    if (fhost_usr_cfg.wifi_ext_flags & WIFI_EXT_PARSE_RX_DATARATE_EN_FLAG) {
        if((machdr_ptr->fctl & MAC_FCTRL_TYPE_MASK) == MAC_FCTRL_DATA_T) {
            ...
            if (mac_vif && (mac_vif->index == vif_idx)) {
                rx_data_rate_save(0, (uint8_t *)&sa.array, rxhdr);
            } else {
                rx_data_rate_save(1, (uint8_t *)&sa.array, rxhdr);
            }
        }
    }
    #endif
    ...
    #if (PLF_AIC8800M40)
    if (fhost_usr_cfg.wifi_ext_flags & WIFI_EXT_PARSE_RX_VECT_EN_FLAG) {
        sta_rx_stat_set((uint8_t *)sa.array, &info->vect);
    }
    #endif
