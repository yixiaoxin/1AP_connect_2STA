...
#define USER_LOFT_CALIB_DISABLE_FLAG    CO_BIT(6)
#define USER_CAPA_CALIB_DISABLE_FLAG    CO_BIT(7) // capa cal depends on loft cal
#define USER_PWR_CALIB_DISABLE_FLAG     CO_BIT(8)
#define USER_MULTI_SCAN_PRBREQ_EN_FLAG  CO_BIT(9)
#define USER_RF_WITH_SAW_EN_FLAG        CO_BIT(10)
#define USER_SETCH_LOFT_CALIB_EN_FLAG   CO_BIT(11)
#define USER_SETCH_RXDC_CALIB_EN_FLAG   CO_BIT(12)
#define USER_IPA_CALIB_DISABLE_FLAG     CO_BIT(13)
...
struct mm_get_sta_info_req
{
    u8_l sta_idx;
    #if (PLF_AIC8800M40)
    uint8_t pattern[3];//used to compatible with old driver
    #endif
};
...
#if (PLF_AIC8800M40)
//more_info
struct mm_get_sta_info_cfm_new
{
    uint32_t rate_info;
    uint32_t txfailed;
    uint8_t rssi;
    uint8_t reserved[3];
    uint32_t chan_time;
    uint32_t chan_busy_time;
    uint32_t ack_fail_stat;
    uint32_t ack_succ_stat;
    uint32_t chan_tx_busy_time;
};
#endif
