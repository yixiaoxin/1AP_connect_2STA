...
def pack_image_lzma(filename, version, addr):
    try:
	    import pylzma
    except ImportError:
...
if __name__ == "__main__":
    ...
    image_vers = sys.argv[2]
    print '<-input-> image_addr: ', image_addr
    print '<-input-> image_vers: ', image_vers
    
    addr_up = int(image_addr, 16)
    print '<-instr-> image_addr:  %08x' % addr_up

    pack_image_lzma('host_wb_upg.bin', image_vers, addr_up)