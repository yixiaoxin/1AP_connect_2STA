...
#include "system.h"
#include "bt_hci.h"
#include "bt_aic8800m80x2_drv_config.h"
#include "dbg.h"
...
const void* aic_fw_ptr_get(enum aic_fw name)
{
    void *ptr = NULL;
    switch(name){
        case FW_ADID_8800D80X2:
            ptr = &fw_adid_u05[0];
            break;
        case FW_PATCH_8800D80X2:
            ptr = &fw_patch_u05[0];
            break;
        case FW_PATCH_8800D80X2_EXT:
            ptr = &fw_patch_u05_ext0[0];
            break;
        case FW_PATCH_TABLE_8800D80X2:
            ptr = &fw_patch_table_u05[0];
            break;
        default:
            dbg("%s ptr is NULL\n",__func__);
            break;
...
int aic_fw_size_get(enum aic_fw name)
{
    ...
    switch(name){
        case FW_ADID_8800D80X2:
            size = AICBT_FW_ADID_U05_SIZE;
            break;
        case FW_PATCH_8800D80X2:
            size = AICBT_FW_PATCH_U05_SIZE;
            break;
        case FW_PATCH_8800D80X2_EXT:
            size = AICBT_FW_PATCH_U05_EXT0_SIZE;
            break;
        case FW_PATCH_TABLE_8800D80X2:
            size = AICBT_FW_PATCH_TABLE_U05_SIZE;
            break;
        default:
            dbg("%s size is 0\n",__func__);
            break;
