...
#if defined(CFG_WIFI_TESTMODE) || PLF_BT_TESTMODE || defined(CFG_DPD)
...
void rw_main(void)
{
    ...
    #endif
    #if PLF_BT_TESTMODE
    if (boot_addr != 0 && ((boot_addr&0x80000000) && (boot_addr & 0x7fffffff)))
    {
    ...
    #endif
    #if defined(CFG_DPD)
    rf_misc_ram_t rf_misc_val;
    int ret = flash_wifi_dpd_info_read(&rf_misc_val);
