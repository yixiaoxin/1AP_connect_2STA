...
void pwrkey_devmode_irq_evt_hndlr(int evt)
{
    PWRKEY_DEBUG_PRINTF("pwrkey:%x\n", evt);
    if ((evt == PWRKEY_IRQ_EVENT_KEYUP) ||
        (evt == PWRKEY_IRQ_EVENT_KEYUP_SINGLE)) {
        PWRKEY_DEBUG_PRINTF("devmode shutdown\n");
        pmic_chip_shutdown_ram();
    }
}
#endif
...
void pwrkey_init(void)
{
    ...
    if (user_devmode_pwrkey_enabled()) {
        pwrkey_irq_callback_register(pwrkey_devmode_irq_evt_hndlr);
