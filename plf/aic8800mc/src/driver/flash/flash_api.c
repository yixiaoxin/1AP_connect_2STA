...
#define CONFIG_FLASH_BLOCK_PROTECT      0
...
#if CONFIG_FLASH_BLOCK_PROTECT
__RAMTEXT void flash_block_set_protected(int bp_en)
{
    while (!ipc_g1_mutex_get(IPC_G1_MUTEX_ID_FLASH_ACCESS));
    if (aonsysctrl_hclkrs_cpuswset_getb()) {
        if (ipc_app2emb28_sel_getf()) {
    ...
    flash_block_protect_all_set(bp_en);
    ipc_g1_mutex_set(IPC_G1_MUTEX_ID_FLASH_ACCESS, 1);
}

__RAMTEXT int flash_block_get_protected(void)
{
    int ret;
    while (!ipc_g1_mutex_get(IPC_G1_MUTEX_ID_FLASH_ACCESS));
    if (aonsysctrl_hclkrs_cpuswset_getb()) {
    ...
    ret = flash_block_protect_all_get();
    ipc_g1_mutex_set(IPC_G1_MUTEX_ID_FLASH_ACCESS, 1);
    return ret;
}
#endif
...
int flash_erase(void *a4k, unsigned int len)
{
    ...
    #if CONFIG_FLASH_BLOCK_PROTECT
    #if (CONFIG_FLASH_CMD_PROTECT_EN)
    flash_block_set_protected(0);
    #else
    flash_block_protect_all_set(0);
    #endif
    #endif
    ...
    #if CONFIG_FLASH_BLOCK_PROTECT
    #if (CONFIG_FLASH_CMD_PROTECT_EN)
    flash_block_set_protected(1);
    #else
    flash_block_protect_all_set(1);
    #endif
    #endif
    return ret;
}
...
#define FLASH_RESERVED_SECTOR_CNT   (5)

#define FLASH_WIFI_DPD_OFFSET       (0 * FLASH_SECTOR_SIZE)
#define FLASH_BTDM_INFO_OFFSET      (1 * FLASH_SECTOR_SIZE)
#define FLASH_WIFI_INFO_OFFSET      (2 * FLASH_SECTOR_SIZE)
#define FLASH_CALIB_INFO_OFFSET     (3 * FLASH_SECTOR_SIZE)
#define FLASH_BOOT_INFO_OFFSET      (4 * FLASH_SECTOR_SIZE)
...
#define FLASH_WIFI_DPD_MAGIC_NUM    0x44504457  /* "WDPD" */
...
{
    int ret = flash_erase((void *)(sys_info_flash_addr + FLASH_WIFI_DPD_OFFSET), FLASH_SECTOR_SIZE);
    dbg("ret %d\r\n", ret);
    flash_cache_invalid_range((void *)(sys_info_flash_addr + FLASH_WIFI_DPD_OFFSET), FLASH_SECTOR_SIZE);
}
int flash_wifi_dpd_info_read(rf_misc_ram_t *rf_misc)
{
    ...
    //unsigned int info_flag_bits;
    wifi_dpd_info_t *wifi_dpd_info = rtos_malloc(sizeof(wifi_dpd_info_t));
    if (!wifi_dpd_info) {
        return INFO_FLAG_INVALID;
    }
    if (!sys_info_inited) {
    ...
    flash_read((void *)(sys_info_flash_addr + FLASH_WIFI_DPD_OFFSET), sizeof(wifi_dpd_info_t), (void *)wifi_dpd_info);
    if (wifi_dpd_info->magic_num == FLASH_WIFI_DPD_MAGIC_NUM) {
        memcpy((char *)rf_misc, &wifi_dpd_info->rf_misc_calib_val, sizeof(rf_misc_ram_t));
        index += sizeof(rf_misc_ram_t);
        if (index > sizeof(rf_misc_ram_t)) {
            rtos_free(wifi_dpd_info);
            return INFO_FLAG_INVALID;
        }
    } else {
        rtos_free(wifi_dpd_info);
