...
void pmic_lpmode_init(void)
{
    ...
    uint32_t vcore08_dslp_vbit, vcore08_dslp_delta;

    // 0x70001040
    vrtc1_ds_sw = (msadc_cal_res.cal_res_flags & CAL_RES_VRTC1_LP_REG_VALID) ? msadc_cal_res.vrtc1_lp_reg : 0xC;
    psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vrtc08_ctrl),
        ...
        PMU_RTC1_RTC_RG_LDO_VRTC08_DVFS_STEP_EN));
    // vcore drop
    vcore08_dslp_delta = (CAL_VCORE_TARGET > 700) ? (CAL_VCORE_TARGET - 700) / 18 : 0;
    vcore08_dslp_vbit = (psim_read((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vcore08_ctrl)) &
        PMU_RTC1_RTC_RG_LDO_VCORE08_VOSEL_NOR_TUNE_MASK) >>
        PMU_RTC1_RTC_RG_LDO_VCORE08_VOSEL_NOR_TUNE_LSB;
    if (vcore08_dslp_vbit > vcore08_dslp_delta) {
        vcore08_dslp_vbit = vcore08_dslp_vbit - vcore08_dslp_delta;
    }
    // 0x7000103C
    vcore08_ds_sw = (syscfg_predefined.pmic_vcore_drop_en) ? vcore08_dslp_vbit : 0xC;
    psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_ldo_vcore08_ctrl),
    ...
    // 0x700021cc
    psim_mask_write((unsigned int)(&AIC_DCDC_RFCTRL->dcdc_ctrl_cfg39),
    (DUAL_DCDC_RF_CTRL_RG_DCDC_EN_NMOS_PMOS_SW_MODE), //nmos_pmos_sw_mode
    (DUAL_DCDC_RF_CTRL_RG_DCDC_EN_NMOS_PMOS_SW_MODE));
}
...
static void pmic_slplvl_hibernate_config(void)
{
    ...
    if (ChipIdGet(2) >= 4) { // >= U04
        ...
        if (syscfg_predefined.sys_hib_lowramret_en) {
            ...
            rsvd_bit = (1<<4) | (0<<2); // hram & lram power down
        }
        // 0x70001088
        psim_mask_write((unsigned int)(&AIC_PMU_RTC1->pmu_cm_xtal_cfg2),
            (PMU_RTC1_RTC_RG_PMIC_RSVD_BIT(rsvd_bit)),
