...
static CDC_HandleTypeDef CdC_HandleType;
...
static USBH_StatusTypeDef USBH_CDC_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  //USBH_DbgLog("firtst interface %d",status);
  ...
  phost->pActiveClass->pData = &CdC_HandleType;//(CDC_HandleTypeDef *)USBH_malloc(sizeof(CDC_HandleTypeDef));
  ...
  interface = USBH_FindInterface(phost, DATA_INTERFACE_CLASS_CODE,
     ...
     //interface = USBH_FindInterface(phost, USER_USB_CDC_CLASS,
       //                            2,1);
...
static void CDC_ProcessTransmission(USBH_HandleTypeDef *phost)
{
  ...
  USBH_UsrLog("CDC_ProcessTransmission");

  switch (CDC_Handle->data_tx_state)
  {
    case CDC_SEND_DATA:	
	  USBH_UsrLog("CDC_SEND_DATA %d",CDC_Handle->DataItf.OutPipe);
