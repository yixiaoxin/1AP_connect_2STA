...
void msadc_input_sel_vref(void)
{
    uint32_t reg_val;
    reg_val = AIC_MSADC->cfg_msadc_sw_ctrl1 & ~(MSADC_REG_CFG_MSADC_SW_MUX_BITS_MASK | MSADC_REG_CFG_MSADC_SW_DIFF_MODE);
...
void msadc_cal_vrf_lp_vref(void)
{
    ...
    vrf13_1 = 1024;
    vrf13_2 = (msadc_volt_measure(MSADC_VOLT_VREF))*1024/760;
    ...
    vrf13_3 = CAL_AVDD13_TARGET > 1000 ? CAL_AVDD13_TARGET - 1000 : 0x0;

    dslp_vrf13_ofst = (vrf13_3 + vrf13_2 - vrf13_1  - 13) / 25;
    dslp_vrf13_reg = vrf13_reg > dslp_vrf13_ofst ? vrf13_reg - dslp_vrf13_ofst : 0xa;
    dbg("vrf1=%d, vrf2=%d, vrf3=%d, nor=0x%x, ofst=%d, ds=0x%x\n",vrf13_1, vrf13_2, vrf13_3,vrf13_reg, dslp_vrf13_ofst, dslp_vrf13_reg);

    if ((dslp_vrf13_reg >= 4) && (dslp_vrf13_reg <= 18)) {
...
void msadc_cal_vrtc1(void)
{
    ...
    if (ChipIdGet(2) >= 4) {
        vrtc1_reg_ofst = (830 - vrtc1_mv) / 14;
        if (vrtc1_reg_ofst < -2) {
            vrtc1_reg_ofst = -2;
        } else if (vrtc1_reg_ofst > 6) {
            vrtc1_reg_ofst = 6;
        }
        msadc_cal_res.vrtc1_lp_reg = (uint8_t)(0xC + vrtc1_reg_ofst);
    } else {
        vrtc1_reg_ofst = (800 - vrtc1_mv) / 14;
        if (vrtc1_reg_ofst < -2) {
            vrtc1_reg_ofst = -2;
        } else if (vrtc1_reg_ofst > 6) {
            vrtc1_reg_ofst = 6;
...
int msadc_volt_measure(int vst)
{
    ...
    if (vst == MSADC_VOLT_VBAT) {
        ...
        ms_val = msadc_ms();
        ret = (int)ms_val * 1214 / MSADC_DENOM_REC - 1214;// Convert to mV
    } else if (vst == MSADC_VOLT_VREF) {
        unsigned int ms_val;
        msadc_basic_config();
        msadc_ad_config();
        msadc_input_sel_vref();
        // msadc_param.mult = 1.0;
        // ms_mV = (ms_val / (float)MSADC_DENOM_REC - 1) * 1214 * msadc_param.mult;
        ms_val = msadc_ms();
        ret = (int)ms_val * 1214 / MSADC_DENOM_REC - 1214;// Convert to mV
