 ...
 * |    I2CM_SCL    |    PA0    |    PA6    |    PA14   |    PB0    |
 * |    I2CM_SDA    |    PA1    |    PA7    |    PA15   |    PB1    |
 * ------------------------------------------------------------------
...
#if I2CM_DMA_ISR
#include "rtos_al.h"
rtos_semaphore i2cm_txdma_sema = NULL;
rtos_semaphore i2cm_rxdma_sema = NULL;
#endif
...
void i2cm_init(int i2cm_chan)
{
    switch(i2cm_chan) {
        ...
        case(I2CM_CHANNAL_3):
            ...
            break;
        case(I2CM_CHANNAL_4):
            iomux_gpiob_config_sel_setf(0, 2);
            iomux_gpiob_config_sel_setf(1, 2);
            break;
        default:
...
void i2cm_deinit(void)
{
    ...
    if (i2cm_txdma_sema) {
        ...
        i2cm_txdma_sema = NULL;
    }
    if (i2cm_rxdma_sema) {
        rtos_semaphore_delete(i2cm_rxdma_sema);
        i2cm_rxdma_sema = NULL;
    }
...
int i2cm_addr_wr(uint8_t waddr, uint8_t wdata)
{
    uint32_t rdata;
...
int i2cm_dma_wr(uint32_t i2c_wlen, uint32_t *wbuff)
{
...
int i2cm_dma_rd(uint32_t i2c_rlen, uint32_t *rbuff)
{
    uint32_t reg;
