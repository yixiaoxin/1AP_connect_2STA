...
void gpioa_force_pull_high(int gpidx)
{
    if (gpidx < 16) {
        iomux_gpioa_config_oen_frc_setf(gpidx, 1);
        iomux_gpioa_config_out_frc_setf(gpidx, 1);
        iomux_gpioa_config_out_reg_setf(gpidx, 1);
    }
}

void gpioa_force_pull_low(int gpidx)
{
    if (gpidx < 16) {
        iomux_gpioa_config_oen_frc_setf(gpidx, 1);
        iomux_gpioa_config_out_frc_setf(gpidx, 1);
        iomux_gpioa_config_out_reg_setf(gpidx, 0);
    }
}

gpio_irq_handler_t gpioa_irq_handler[GPIOA_IDX_MAX + GPIO_USB_MAX] = {NULL};
...
{
    if (gpidx < (GPIOA_IDX_MAX + GPIO_USB_MAX)) {
        unsigned int gpmsk = 0x01UL << gpidx;
...
void gpioa_isr(void)
{
    ...
    while (int_status) {
        ...
        if (bit >= (GPIOA_IDX_MAX + GPIO_USB_MAX)) {
            // error occors
            break;
        }
        ...
        if (gpioa_irq_handler[bit]) {
            ...
            } else { //GPIOIRQ_EVENT_EDGE_FALL
                int reg_val = AIC_GPIOA->TLR;
                AIC_GPIOA->TLR = ~reg_val;
            }
...
void gpiob_force_pull_high(int gpidx)
{
    if (gpidx < GPIOB_IDX_MAX) {
        iomux_gpiob_config_oen_frc_setf(gpidx, 1);
        iomux_gpiob_config_out_frc_setf(gpidx, 1);
        iomux_gpiob_config_out_reg_setf(gpidx, 1);
    }
}

void gpiob_force_pull_low(int gpidx)
{
    if (gpidx < GPIOB_IDX_MAX) {
        iomux_gpiob_config_oen_frc_setf(gpidx, 1);
        iomux_gpiob_config_out_frc_setf(gpidx, 1);
        iomux_gpiob_config_out_reg_setf(gpidx, 0);
    }
}
