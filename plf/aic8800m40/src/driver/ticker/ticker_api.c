...
extern void lp_ticker_read_time(uint32_t *msb, uint32_t *lsb);
...
static void initialize(const ticker_data_t *ticker)
{
    ...
    if (ticker->interface->runs_in_deep_sleep) {
        uint32_t us_hi, us_lo;
        lp_ticker_read_time(&us_hi, &us_lo);
        ticker->queue->present_time = ((uint64_t)us_hi << 32) + us_lo;
    } else {
