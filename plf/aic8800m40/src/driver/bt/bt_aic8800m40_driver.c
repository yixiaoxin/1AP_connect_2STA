...
************************************************************/
#define AICBT_BTMODE_DEFAULT        AICBT_BTMODE_BT_ONLY//_COANT
...
void aic_bt_start(void)
{
    ...
    if(aicbt_info.btmode == AICBT_BTMODE_BT_ONLY_COANT){
        ...
        pwrctrl_wificore_sleep_src_setf((0x01UL << 1));
        pwrctrl_wificore_awake_src_setf((0x01UL << 1));
    }
    #endif
...
int bt_drv_enter_uart_dut_test(void)
{
    PMIC_MEM_WRITE((unsigned int)&AIC_PMU_RTC0->rtc_rg_reserved0, 0x80000000);
...
//flag: 1 off fw print
//      0 on fw print
void bt_drv_set_fw_print_off(uint32_t flag)
{
    uint8_t rom_ver;
    uint32_t chipid;
#if (CFG_ROM_VER == 255)
...
#endif
    chipid = ChipIdGet(1);
    dbg("%s ,rom ver %d,chipid %d\n",__func__,rom_ver,chipid);
    switch (rom_ver) {
        case 0:
        case 1:
        case 2:
            *(volatile uint32_t *)(0x201dbc) = flag;
            break;
        case 3:
            *(volatile uint32_t *)(0x201e7c) = flag;
            break;
        default:
            break;
    }
