...
void appm_set_bt_addr(uint8_t * addr)
{
    struct gap_bdaddr *cmd =
            KE_MSG_ALLOC(GAPM_SET_BT_ADDR_CMD, TASK_GAPM, TASK_APP,  gap_bdaddr);
...
void app_ble_get_rssi(uint8_t conidx)
{
    struct gapc_get_info_cmd *p_cmd = KE_MSG_ALLOC(GAPC_GET_INFO_CMD,
                                                   KE_BUILD_ID(TASK_GAPC, conidx), KE_BUILD_ID(TASK_APP, conidx),
                                                   gapc_get_info_cmd);
    
    // request peer device features.
