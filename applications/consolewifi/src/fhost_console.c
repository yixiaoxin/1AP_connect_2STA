...
static rtos_task_handle fhost_console_handle;
        ...
        for (i = 0; i < status.vif_max_cnt; i++) {
            ...
            if (vif_status.type != VIF_UNKNOWN) {
                if (vif_status.chan.prim20_freq != 0) {
                    dbg(", Operating Channel: %dMHz(center1=%dMHz)@%dMHz\n",
                                vif_status.chan.prim20_freq,
                                vif_status.chan.center1_freq,
...
#define CONSOLE_DEEPSLEEP_IOA5_INDEX  5
...
int do_enter_hibernate(int argc, char * const argv[])
{
    ...
    if (level >= PM_LEVEL_LIGHT_SLEEP) {
        ...
        #endif /* WAKEUP_IO_A */
        #ifdef WAKEUP_IO_A_FALL_EDGE
        // gpioa5 as wake src
        gpioa_irq_init(CONSOLE_DEEPSLEEP_IOA5_INDEX, GPIOIRQ_TYPE_EDGE_FALL, gpio_irq_test_handler);
        user_sleep_wakesrc_set(WAKESRC_GPIOA, 1, WAKEGPIO_ARG(WAKEGPIO_MUX_1, CONSOLE_DEEPSLEEP_IOA5_INDEX));
        #endif /* WAKEUP_IO_A_FALL_EDGE */
        #endif /* PLF_AIC8800M40 */
...
static RTOS_TASK_FCT(tcp_wakeup_task)
{
...
#if NONBLOCK_ENABLE
    {
        ...
        FD_ZERO(&write_sets);
        FD_SET(sock_tcp_wakeup, &write_sets);

        timeselect.tv_sec = 30;
        timeselect.tv_usec = 0;

        if (connect(sock_tcp_wakeup, (struct sockaddr*)&addr, sizeof(addr)) == 0)
        {
            int error;
            socklen_t len = sizeof(error);
            if (getsockopt(sock_tcp_wakeup, SOL_SOCKET, SO_ERROR, &error, &len) == 0 && error == 0)
            {
                dbg("connect success\n");
            }
            else
        ...
        {
            ret = select(sock_tcp_wakeup + 1, NULL, &write_sets, NULL, &timeselect);
            if (ret == 0 )
            {
                dbg("connect timeout\n");
                goto exit;
        ...
        if (argc > 4) {
            ...
            if (wlan_get_connect_status() == WLAN_DISCONNECT) {
                if (fvif_status.type == VIF_UNKNOWN) {
                    fhost_set_vif_type(sta_cntrl_link, fvif_idx, VIF_UNKNOWN, false);
                }
                fhost_cntrl_cfgrwnx_link_close(sta_cntrl_link);
            }
            #endif
...
void test_rtos_timer_handler(void const *param)
{
    uint32_t rtos_tick;
    uint32_t msb, lsb;
    rtos_tick = rtos_now(false);
    lp_ticker_read_time(&msb, &lsb);
    dbg("tmr=%d ms, lptk=%u (%x,%08x) us, ", rtos_tick, lsb, msb, lsb);
    print_rtc_time_info();
    uart_wait_tx_done();
}

TimerHandle_t test_rtos_timer = NULL;

static int do_rtos_timer_test(int argc, char * const argv[])
{
    int peroid_sec = console_cmd_strtoul(argv[1], NULL, 10);
    if (test_rtos_timer == NULL) {
        test_rtos_timer = rtos_timer_create("test_rtos_timer", peroid_sec * 1000, pdTRUE, NULL,
                                            (TimerCallbackFunction_t)test_rtos_timer_handler);
        if (!test_rtos_timer) {
            dbg("test_rtos_timer create fail\n");
            return -1;
        }
        rtos_timer_start(test_rtos_timer, 100, false);
        dbg("start rtos_timer with period %d s\n", peroid_sec);
    } else {
        rtos_timer_change_period(test_rtos_timer, peroid_sec * 1000, 0);
        dbg("re-set rtos_timer period to %d s\n", peroid_sec);
    }
    return 0;
}

#if (PLF_AIC8800M40)
static int do_get_cs_info(int argc, char * const argv[])
{
    ...
    dbg("phymode=%d(0:B 1:G 2:A 3:N 4:AC 5:AX)\n", cs_info.phymode);
    dbg("bandwidth=%d(0:20 1:40 2:80)\n", cs_info.bandwidth);
    dbg("freq=%d\n", cs_info.freq);
    dbg("rssi=%d\n", cs_info.rssi);
    dbg("snr=%d\n", cs_info.snr);
    dbg("noise=%d\n", cs_info.noise);
...
static void fhost_command_add(void)
{
    ...
    console_cmd_add("get_cs_info", "wifi get_cs_info", 2,  8, do_get_cs_info);
    #endif
    console_cmd_add("rtmr",  "rtos timer test", 2, 2, do_rtos_timer_test);
    #if NX_FHOST_MONITOR
...
rtos_task_handle dut_get_console_handle(void)
{
    return fhost_console_handle;
}
...
int fhost_application_init(void)
{
    ...
    if (rtos_task_create(fhost_console_task, "CONSOLE", APPLICATION_TASK,
                         1024, NULL, RTOS_TASK_PRIORITY(1), &fhost_console_handle)) {
        return 1;
    }
