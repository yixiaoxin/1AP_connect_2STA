...
struct fhost_cntrl_link *console_cntrl_link;
...
int do_start_p2p(int argc, char * const argv[])
{
    ...
    if (argc > 1) {
        freq = console_cmd_strtoul(argv[1], NULL, 10);
    }
    ...
    if (freq < 5000) {
        ...
        p2p_cfg.chan.prim20_freq = freq;
        p2p_cfg.chan.center1_freq = freq;
    }
    else {
 ...
 ****************************************************************************************
 */
int do_scan(int argc, char * const argv[])
{
    int nb_res;
    ...
    while(fhost_get_scan_results(console_cntrl_link, nb_res++, 1, &result)) {
        ...
        dbg("%2d:(%3d dBm) CH=%3d AKM=%3x G=%3x P=%3x S=%2d BSSID=%02x:%02x:%02x:%02x:%02x:%02x "
            ...
            wlan_akm2security(result.akm, result.group_cipher, result.pairwise_cipher),
...
int do_iperf(int argc, char * const argv[])
{
