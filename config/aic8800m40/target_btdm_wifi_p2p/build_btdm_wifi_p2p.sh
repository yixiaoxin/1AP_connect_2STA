...
python ../../../tools/scons.py -u . PRODUCT=host-wb PLF=aic8800m40 BT=armgcc_4_8 BUILD_CMD="$0 $*" \
	TGT_MODE=btdm_wifi_p2p TGTNAME=btdm_wifi_p2p \
	BT_BLE=on BTDM=on APPS=on A2DP=off A2DP_SOURCE=off HFP_AG=off LINK_ONE=on AUD_USED=off \
    BLE_DFT_STATE=none BLE_APP_AUDTRANSMIT=none BLE_APP_SMARTCONFIG=off BLE_APP_HID=off BLE_APP_UDF=all_role \
	BAND5G=on SOFTAP=on IPERF=on PING=on CHKSUM=off WIFI_RAM_VER=on P2P=on \
	HEAP_SIZE=0x30000 FHOST_APP=console $opt
