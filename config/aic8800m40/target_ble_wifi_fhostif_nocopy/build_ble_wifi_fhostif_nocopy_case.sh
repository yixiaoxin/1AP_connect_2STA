...
python ../../../tools/scons.py -u . PRODUCT=host-wb PLF=aic8800m40 BT=armgcc_4_8 BUILD_CMD="$0 $*" AON=off \
        TGT_MODE=ble_wifi_fhostif_nocopy \
        BT_BLE=on BT_AUDIO=off APPS=on BLE_ONLY=on BT_USER=on TGTNAME=ble_wifi_fhostif_nocopy \
        BLE_DFT_STATE=adv BLE_APP_AUDTRANSMIT=none BLE_APP_SMARTCONFIG=off BLE_APP_HID=off BLE_APP_UDF=all_role \
        AUDIO=off UTILS=on HW_SBC=off AAC=off BT_PROMPT=off HOSTIF_OPT=on \
        SOFTAP=on PING=on IPERF=on CHKSUM=off HEAP_SIZE=0x20000 WIFI_RAM_VER=on \
