...
python ../../../tools/scons.py -u . PRODUCT=host-wb PLF=aic8800m40 BT=armgcc_4_8 BUILD_CMD="$0 $*" AON=off \
        TGT_MODE=bt_wifi_fhostif \
        TGTNAME=bt_wifi_fhostif BT_AUDIO=on AUD_USED=on APPS=on A2DP=off A2DP_SOURCE=off HFP_AG=on \
        SOFTAP=on PING=on IPERF=on CHKSUM=off \
        HEAP_SIZE=0x20000 WIFI_RAM_VER=on \
        FHOST_APP=fhostif  HOSTIF=usb BAND5G=on CONSOLE=on $opt
