...
python ../../../tools/scons.py -u . PRODUCT=host-wifi PLF=aic8800mc BT=armgcc_4_8 BUILD_CMD="$0 $*" \
    SOFTAP=off FHOST_APP=fhostif AON=on BAND5G=off ROM_VER=auto PING=on IPERF=on FHOSTIF_IPC_FC=on \
    TGT_MODE=wifi_fhostif_rateopt CHKSUM=off WIFI_RAM_VER=off HOSTIF=usb HEAP_SIZE=0xf800 $opt
