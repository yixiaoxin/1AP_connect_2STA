...
def env_init():
    ...
    # External crypto library (available value: none, mbedtls_v2_16_2, mbedtls_v3_6_2)
    if env['RTOS'] in ['freertos'] :
        env['CRYPTO'] = ARGUMENTS.get('CRYPTO', 'mbedtls_v2_16_2').lower()
    else :
    ...
    #DPD
    if env['PLF'] in ['aic8800m40', 'aic8800mc']:
        env['DPD'] = ARGUMENTS.get('DPD', 'on').lower()
    else:
        env['DPD'] = ARGUMENTS.get('DPD', 'off').lower()

    #Dynamic DPD calib when temperature changes, cal_res stored in flash
    env['DYNAMIC_DPD'] = ARGUMENTS.get('DYNAMIC_DPD', 'off').lower()
    ...
    # Wi-Fi rftest
    env['WIFI_RFTEST'] = ARGUMENTS.get('WIFI_RFTEST', 'off')

    # Flash fw
    env['FLASH_FW'] = ARGUMENTS.get('FLASH_FW', 'off')

    # wifi tx task
    env['WIFI_TX_TASK'] = ARGUMENTS.get('WIFI_TX_TASK', 'on')
