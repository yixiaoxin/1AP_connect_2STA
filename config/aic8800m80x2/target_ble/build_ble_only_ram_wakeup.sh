...
# BLE_APP_HID         : off/on
python ../../../tools/scons.py -u . PRODUCT=host-bt PLF=aic8800m80x2 BT=armgcc_4_8 BUILD_CMD="$0 $*" AON=off \
        TGT_MODE=ble \
        BT_BLE=on BT_AUDIO=off APPS=on BLE_ONLY=on BT_USER=on TGTNAME=fw_ble_scan_ad_filter \
        BLE_DFT_STATE=scan BLE_APP_AUDTRANSMIT=none BLE_APP_SMARTCONFIG=off BLE_APP_HID=off\
        AUDIO=off UTILS=on HW_SBC=off AAC=off BT_PROMPT=off CONSOLE=on \
        ROM_VER=auto HEAP_SIZE=0x20000 CODE_START_ADDR=0x00160000 \
        BLE_WAKEUP=on $opt
